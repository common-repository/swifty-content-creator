			...
			editor.on( 'fileUploadResponse', function( evt ) {
				...
				try {
					...
					if ( !response.uploaded ) {
						...
						data.asset_html = response.asset_html;  // added by MK: keep the asset_html for insertion
						evt.stop();                             // added by MK: do not handle by default event handler
					}
				} catch ( err ) {
			...
			xhr.onload = function() {
				...
				if ( xhr.status < 200 || xhr.status > 299 ) {
					...
					var data = {
						...
						valuesToCopy = [ 'message', 'fileName', 'url', 'asset_html' ], // MK added asset_html for swifty
